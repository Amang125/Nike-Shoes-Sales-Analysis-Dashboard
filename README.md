# Nike-Shoes-Sales-Analysis-Dashboard

📁 Project File
File Name: Nike Shoes Dashboard
Tool Used: Power BI

![Overview Page](https://github.com/Amang125/Nike-Shoes-Sales-Analysis-Dashboard/blob/main/Screenshot%202026-01-16%20215534.png)
![Insight Page](https://github.com/Amang125/Nike-Shoes-Sales-Analysis-Dashboard/blob/main/Screenshot%202026-01-16%20215608.png)
![Performance Page](https://github.com/Amang125/Nike-Shoes-Sales-Analysis-Dashboard/blob/main/Screenshot%202026-01-16%20215703.png)














#  Nike Sales Dashboard – Power BI Project  

This project is created to analyze Nike product sales data in a simple and interactive way using Power BI.  
It helps understand sales performance, customer behavior, and regional trends.

---

##  What This Dashboard Shows  

✔ Sales performance by region  
✔ Monthly sales trend  
✔ Product performance insights  
✔ Profit summary  
✔ Customer ratings & reviews  
✔ Clean and professional UI  

---

##  Pages Inside Dashboard  

### ➤ Overview Page  
Shows product showcase and sales summary  

### ➤ Insight Page  
Shows product details like price, rating, and reviews  

### ➤ Performance Page  
Shows region-wise sales, monthly trends, and profit  

---

##  Key Learnings From This Project  

- Northeast and West regions generate highest sales  
- July month has the highest sales  
- Air Jordan 1 Mid SE is the best performing product  
- Sales follow seasonal patterns  

---

##  Tools Used  

- Power BI  
- DAX  
- Power Query  
- Data Modeling  

---

##  Why I Built This Project  

I created this dashboard to:  

- Improve Power BI skills  
- Practice real business dashboards  
- Build a strong data analytics portfolio  
- Learn data visualization best practices  

---

##  Created By  

**Aman Gole**  
Power BI Developer | Data Analyst  

GitHub: https://github.com/yourusername  
LinkedIn: https://linkedin.com/in/yourprofile  

---

## ⭐ Like This Project? 

If you like this project:  
- Star ⭐ this repository  
- Fork the project  
- Share your feedback 
Follow for more projects  

